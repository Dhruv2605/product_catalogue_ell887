...
@login_required
def clear_products():
    """API to delete all products from Cosmos DB."""
    try:
        ...
        for item in items:
            container.delete_item(item["id"], partition_key=item["id"])  # Assuming id is partition key
        
        logging.info("🗑️ All products have been deleted from Cosmos DB")
